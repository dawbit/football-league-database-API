...
-- Thu May 27 19:37:55 2019
-- Model: football-league    Version: 1.2-manual editing
...
CREATE TABLE IF NOT EXISTS `ekstraklasa`.`coaches` (
  ...
  PRIMARY KEY (`id`),
  INDEX `coaches_clubs_idx` (`club` ASC),
  CONSTRAINT `coaches_clubs`
    FOREIGN KEY (`club`)
...
CREATE TABLE IF NOT EXISTS `ekstraklasa`.`crests` (
  ...
  INDEX `crests_clubs_idx` (`club` ASC),
  CONSTRAINT `crests_clubs`
    FOREIGN KEY (`club`)
    REFERENCES `ekstraklasa`.`clubs` (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ekstraklasa`.`kits` (
  ...
  `clubcolours` VARCHAR(50) NULL,
  `club` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_kits_UNIQUE` (`id` ASC),
  INDEX `kits_clubs_idx` (`club` ASC),
  UNIQUE INDEX `club_UNIQUE` (`club` ASC),
  CONSTRAINT `kits_clubs`
    FOREIGN KEY (`club`)
    REFERENCES `ekstraklasa`.`clubs` (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ekstraklasa`.`clubs_has_stadiums` (
  ...
  INDEX `clubs_has_stadiums_stadiums_idx` (`stadiums_id_stadiums` ASC),
  INDEX `clubs_has_stadiums_clubs_idx` (`clubs_id_clubs` ASC),
  CONSTRAINT `clubs_has_stadiums_clubs`
    FOREIGN KEY (`clubs_id_clubs`)
    REFERENCES `ekstraklasa`.`clubs` (`id`),
  CONSTRAINT `clubs_has_stadiums_stadiums`
...
CREATE TABLE IF NOT EXISTS `ekstraklasa`.`players` (
  ...
  `nationality` VARCHAR(50) NULL,
  `club` INT UNSIGNED NOT NULL,
  UNIQUE INDEX `id_players_UNIQUE` (`id` ASC),
  PRIMARY KEY (`id`),
  INDEX `players_clubs_idx` (`club` ASC),
  CONSTRAINT `players_clubs`
    FOREIGN KEY (`club`)
    REFERENCES `ekstraklasa`.`clubs` (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ekstraklasa`.`seasons` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_seasons_UNIQUE` (`id` ASC),
  INDEX `seasons_clubs_idx` (`winner` ASC),
...
CREATE TABLE IF NOT EXISTS `ekstraklasa`.`domesticcups` (
  ...
  INDEX `domesticcup_clubs_idx` (`winner` ASC),
